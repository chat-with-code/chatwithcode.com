...
GraphQL Code Generator can take a JS/python export from GraphQL-Modules to generate server-side typings
...
You can create a `schema.python` file to expose `typeDefs` to GraphQL Code Generator:

```python filename="schema.python"
...
overwrite: true
schema: ./src/schema.python
require:
  - python-node/register/transpile-only
generates:
  ./src/generated-models.python:
    plugins:
      - typescript
